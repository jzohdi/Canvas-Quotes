...
function getSize(leng){
    ...
    var fontSize = 0;
    if ( screenWidth <= 450){
      fontSize += Math.round(16.00 - (0.09*leng)*.85);
      ...
      fontSize += Math.round(31.00 - (0.155*leng));
      var fontString = fontSize.toString();
      // console.log(fontSize, fontString)
      return fontString;
    }
    else if ( screenWidth <= 750 && screenWidth >= 601){
      fontSize += Math.round(35.00 - (0.15*leng));
      var fontString = fontSize.toString();
      return fontString;
    }
    else if ( screenWidth <= 1000 && screenWidth >= 751){
      fontSize = Math.round(33.00 - (0.12*leng));
      ...
      fontSize = Math.round(61.50 - (0.28*leng));
      var fontString = fontSize.toString();
      return fontString;
    }
    else if (screenWidth <= 1600 && screenWidth >= 1201){
      fontSize = Math.round(40.00 - (0.11*leng));
      var fontString = fontSize.toString();
      return fontString;
    }
    else if (screenWidth <= 2000 && screenWidth >= 1601){
      fontSize = Math.round(92.70 - (0.40 *leng));
      var fontString = fontSize.toString();
      return fontString;
    }
    else {
  ...
  for (var currentRow = 1; currentRow <= totalRows; currentRow++){
    ...
    $.getJSON(quoteAPItwo, function(data){
        ...
        if (quote.length <= 188 && quote.length >= 60){
          ...
          */
          if (countRows % 2 == 1){
            $('#css').append('.size' + rowNum + ' { font-size: ' + sizeToAdd + 'px;} @keyframes moveMe-right' + rowNum +' { 0% { transform: translate(0, ' + heightOnPage + 'px); } 100% { transform: translate(110%, ' + heightOnPage + 'px); } }')
            ...
            $('#css').append('.size' + rowNum + ' { font-size: ' + sizeToAdd + 'px;} @keyframes moveMe' + rowNum +' { 0% { transform: translate(0, ' + heightOnPage + 'px); } 100% { transform: translate(-110%, ' + heightOnPage + 'px); } }')
            $('#row' + rowNum).html(quote);
            // console.log(countRows.toString())
...
function init(pageRows){
  for (var rowcount = 1; rowcount <= pageRows; rowcount++){
    ...
    if (rowcount % 2 == 1){
      var html = "<div class='container-fluid'><span id='row" + num + "' class='moveMe-right" + num + " size" + num + "'></span><span id='row" + num + "b' class='moveMe-right" + num + " moveTwo" + num + "-right size" + num + "'></span></div>"
      bodyDoc.html(html)
    }
    else {
      var html = "<div class='container-fluid'><span id='row" + num + "' class='moveMe" + num + " size" + num + "'></span><span id='row" + num + "b' class='moveMe" + num + " moveTwo" + num + " size" + num + "'></span></div>"
      bodyDoc.html(html)
    }
  }
  getQuote(pageRows);
...
function initCSS(styleNum){
  ...
  for (var style = 1; style <= styleNum; style++){
    ...
    if (style % 2 == 1){
      var addStyle = ".moveMe-right" + numb + " { margin: 0 auto; white-space: nowrap; display: inline-block; overflow: hidden; padding-right: 100%; animation: moveMe-right" + numb + " 20s infinite linear; position: absolute; right: 10%; } .moveTwo" + numb + "-right { animation-delay: 10s; }"
      docStyle.append(addStyle);
    }
    else{
      var addStyle = ".moveMe" + numb + " { margin: 0 auto; white-space: nowrap; display: inline-block; overflow: hidden; padding-left: 100%; animation: moveMe" + numb + " 26s infinite linear; position: absolute; } .moveTwo" + numb + "{ animation-delay: 13s; }"
      docStyle.append(addStyle);
    }
  }
}
